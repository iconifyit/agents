...
The case the reviewer identified as the real hazard — an inspection whose *effect* repairs the system, where no skip condition engages because nothing is harmed — is now closed by the effect floor in `agents/post-mortem.md`: an action that would repair, restart, unstick, release or clear the system is remediation whatever the intent and is unavailable. If this exception is ever revisited, that floor and condition (b) are the two things holding it up.
