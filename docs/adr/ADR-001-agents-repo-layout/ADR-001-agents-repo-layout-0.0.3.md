...
- `post-mortem` — the same plus `Write, Edit`. It produces versioned documents and needs both. The split is by whether the target file exists, not by whether it is the first: `Write` creates a file that does not yet exist — the first version, each superseding version, and the pointer on first creation — and `Edit` changes one that already does, including the pointer on every subsequent supersession. The definition carries the per-file table; this is the reason the grant has two tools. `Edit` does not make blanking impossible — the whole file as `old_string` and `""` as `new_string` would do it — but it makes blanking require deliberate construction rather than being the default failure mode of a careless `Write`. That is the whole of the argument for the grant, and it is enough.
...
**(b) has already been relaxed once and this paragraph did not catch it.** It originally read "destructive action on production", which if taken as *any irreversible mutation* would have closed step 4 on production entirely; `1ef1e7c` redefined it as loss or disruption, judged by what is lost rather than by technical irreversibility. That is a real widening of what the agent may do on production, it was the right call, and the tripwire below fired on it silently. Treat the tripwire as necessary and not sufficient: it tells you to look, it does not look for you.

Relaxing (b) further reopens the objection, and nothing else in the record would flag it, so it is flagged here.
