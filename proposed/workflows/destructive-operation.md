# DRAFT — proposed workflow `destructive-operation`

> **Status:** draft for review (revised: first-principles, domain-agnostic — not git-specific).
> **Pairs with:** the live `destructive-actions` rule (the boundary) and the proposed
> `destructive-operations` skill (the verify-and-catalog discipline).
>
> **Open questions for Scott:**
> - Keep both a skill *and* this workflow only if their value is genuinely distinct —
>   the skill = the reusable verify-and-catalog discipline, this = the ordered sequence
>   for an actual execution. Collapse to one if that's cleaner.
> - The core principle may live better in the `destructive-actions` rule (see the skill draft).
